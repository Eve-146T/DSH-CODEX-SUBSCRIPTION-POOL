...
export class OpenAICodexAuth extends Service {
    ...
    constructor(ctx, config) {
        ...
        ctx.effect(() => ctx.commands.register({
            ...
            handler: (invocation) => this.loginCommand(invocation),
        }));
        ctx.effect(async () => {
        ...
        ctx.effect(() => ctx.commands.register({
            ...
            handler: (invocation) => this.logoutCommand(invocation),
        }));
    }
    ...
    async loginCommand(invocation) {
        if (invocation.rawInput.trim())
            return { kind: 'error', text: 'Usage: /login-openai' };
        ...
        return { kind: 'success', text: 'Signed in to OpenAI (ChatGPT Plus/Pro).' };
    }
    ...
    async logoutCommand(invocation) {
        if (invocation.rawInput.trim())
            return { kind: 'error', text: 'Usage: /logout-openai' };
        ...
        this.usageCache = undefined;
        this.usageError = undefined;
        return { kind: 'success', text: 'Signed out of OpenAI.' };
    }
    async logout() {
        await withFileLock(this.filename, async () => {
            try {
