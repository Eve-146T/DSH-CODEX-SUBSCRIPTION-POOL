...
export function bundledOpenAiDocsSkill(): string {
...
export function dshOpenAiDocsSkill(): string {
  return `${DSH_SKILL_PREAMBLE}\n\n${bundledOpenAiDocsSkill()}`
}

function advertisedIds(models: readonly LlmModelInfo[]): string[] {
  const ids = models
...
export function renderModelAwareness(models: readonly LlmModelInfo[]): string {
...
export function apply(ctx: Context): void {
  ...
  const refresh = async (): Promise<void> => {
    ...
    try {
      ...
      if (generation === refreshGeneration) models = next
    } catch (error: unknown) {
  ...
  ctx.skills.register({
    name: 'openai-docs',
    description:
      'Use for current, authoritative OpenAI API, model, and Codex guidance with official sources.',
    whenToUse:
      'Use for OpenAI model selection, capabilities, migration, prompting, API behavior, or Codex self-knowledge.',
    source: 'bundled',
